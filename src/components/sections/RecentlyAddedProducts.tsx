...
export const RecentlyAddedProducts: React.FC = () => {
  return <section className="py-12 bg-slate-100">
      <div className="container mx-auto px-4">
        <SectionHeader title="Adicionados Recentemente" description="Confira os produtos mais recentes na nossa loja" actionLabel="Ver Todos" onAction={() => console.log("Ver todos os produtos recentes")} />
        
        <Carousel className="w-full">
          <CarouselContent className="-ml-2 md:-ml-4">
            {recentProducts.map((product, index) => <CarouselItem key={index} className="pl-2 md:pl-4 basis-full xs:basis-1/2 sm:basis-1/2 md:basis-1/3 lg:basis-1/4">
